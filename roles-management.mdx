...
description: "Create role templates and assign them to users at /dashboard/roles"
---

The **Roles** page (`/dashboard/roles`) is where org admins define and manage
...
<Note>
  Only users with the `roles-management` permission (granted by default to
  the built-in **Org Admin** role) can access this page. Everyone else is
...
4. Toggle on the permissions you want the role to grant, organized into four
   ...
   - **MCP Tools** — grouped by category (User Management, Role Management,
     ...
     each toggle grants `execute` access to that specific tool
...
Role assignment happens from the **Users** page (`/dashboard/users`), not
...
## How this maps to the underlying model

Each permission toggle in the role editor corresponds to a specific,
concrete access grant — an organization-level role, a feature flag, MCP
server access, or execute access on one MCP tool. When you assign a role
template to a user, the framework expands those grants for that user's
actual organization and applies them; when you revoke or delete a template,
it computes the net set to remove so a user never loses a permission they
still hold through a different role. You don't need to think about this
mechanics to use the page — it's handled automatically on save.
