...
forwarding the caller's authentication token so results are already filtered
to what that user is authorized to see. Authorization isn't enforced at this
layer — it's enforced downstream by `apps/kbmcpserver`. Degrades gracefully: if the
