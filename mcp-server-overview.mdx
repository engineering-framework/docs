...
<Note>
  There's a second, separate MCP server for the Knowledge Base
  (`apps/kbmcpserver`) — search and retrieval over Knowledge Base Documents,
  not planning or administration. The Framework MCP server's own
