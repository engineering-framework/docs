...
input, builds an `AuthContext` from the caller's authenticated identity, and
delegates the actual work to `@repo/core/operations` — the same business
logic the web frontend uses. There is no separate code path or relaxed
permission model for agent access.
...
│  Claude Desktop)  │  HTTP  │  (apps/mcpserver)  │        │  (data + perms)   │
└─────────────────┘        └──────────────────┘        └─────────────────┘
                                     ▲
                                     │ same AuthContext,
...
- **Authentication**: the agent's MCP client presents a valid bearer token on
  every request (`Authorization: Bearer <token>`), verified using standard
  JWT validation.
- **Authorization**: each tool call is checked against the same permission
  model before it runs. Tools marked `fgaControlled` are hidden from
  `tools/list` entirely if the caller can't view them, and rejected at
  execution time if the caller can't execute them — an agent never sees or
  successfully calls a tool outside the user's actual permissions.
- **Data**: all reads and writes go through the same
