...
<Warning>
  ...
  canonical authorization model definition was found in-repo at
  `scripts/fga-model.mjs` and cross-checked against those call sites, but the
  live OpenFGA store is the actual source of truth at runtime — if your
  organization's behavior doesn't match what's described here, verify against
  the OpenFGA dashboard/store before assuming this doc is stale.
</Warning>
